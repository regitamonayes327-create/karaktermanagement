...
if ($parentId) {
    $children = $db->fetchAll("
        SELECT s.id, s.full_name, s.nis, c.class_name, c.grade_level 
...
// 2. Setup Filters & Selected Child
...
// 3. Level labels, colors, and icons
$levelLabels = [
...
// 4. Get Achievements & Stats (If child exists)
$achievements = [];
$stats = [];
$totalPoints = 0;
$totalAchievements = 0;
...
if ($child) {
    ...
    $achievements = $db->fetchAll("
        SELECT a.*, ac.category_name as ach_cat_name 
        FROM achievements a 
    ...
    $levelStats = $db->fetchAll("
        SELECT level, COUNT(*) as total, SUM(points) as total_points 
        FROM achievements 
        WHERE student_id = ? AND show_to_parent = 1 
        GROUP BY level
    ", [$childId]);
            ...
            <div class="bg-gradient-to-r from-yellow-500 to-orange-500 rounded-xl p-6 mb-6 text-white">
                <div class="flex items-center justify-between flex-wrap gap-4">
                    <div>
                        ...
                        <p class="text-sm text-yellow-100">Kelas <?= htmlspecialchars($child['grade_level'] ?? '-') ?> - <?= htmlspecialchars($child['class_name'] ?? '-') ?></p>
